...
import { Card, Button, Input, cn, MarkdownContent } from '../components/ui'
...
function TableCard({ table, isClosed, onSelect }: { table: any, isClosed: boolean, onSelect: () => void }) {
  ...
  return (
    <Card className={cn("flex flex-col h-full hover:translate-x-[-4px] hover:translate-y-[-4px] transition-transform", isDisabled && "opacity-60")}>
      <div className='flex gap-5'>
        <div>
          {table.imageUrl && (
            <img src={table.imageUrl} alt={table.title} className="w-[220px] h-[220px] object-cover transition-all" />
          )}
        </div>
      ...
      <div className="flex flex-col flex-grow p-4">
        <MarkdownContent content={table.description} className="flex-grow" />

        <Button
          onClick={onSelect}
          className={cn(
            "w-full py-2 text-lg uppercase italic mt-5",
            isDisabled && "bg-gray-400 cursor-not-allowed shadow-none"
          )}
          disabled={isDisabled}
        >
          {isClosed ? 'Cerrado' : (isFull ? 'Mesa Llena' : 'Unirme')}
        </Button>
      </div>
    </Card>
  )
